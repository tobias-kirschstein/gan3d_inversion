...
from gan3d_inversion.goae.training.volumetric_rendering import math_utils
...
def create_camera_for_yaw(yaw_angle: float, distance: float, height: float = 0) -> torch.Tensor:
    position = Vec3(math.sin(yaw_angle), height, math.cos(yaw_angle)) * distance
    pose = Pose(matrix_or_rotation=np.eye(3),
    ...
    c = torch.from_numpy(encode_camera_params(pose, DEFAULT_INTRINSICS))
